...
This started as a **V1 skeleton** and has since gone through four hardening passes (Backlog Stream B: migration & cutover readiness; Backlog Stream A: wiring completion; Backlog Stream E: systemic correctness hardening; Backlog Stream C: security hardening). The ledger-centric core, auth, and the Stock/Purchase Orders/Shipments/Money dashboards are built and tested, the schema/migration tooling is real (foreign keys, negative-stock validation, quarantine-not-abort migration handling), every piece of backend logic V1's own review found "built, tested, but unreachable" now has a real router procedure and UI, the daily Shopify pull is idempotent with a pinned-UTC connection timezone and enforced landed-cost currency matching, and auth now runs on real per-user credentials with session revocation and a per-email login throttle — but it has not yet run against real Control Tower data — see [Current Status](#current-status) below before assuming this is production-ready.
...
- 167 tests passing on the documented `pnpm test` command; `pnpm check` (`tsc --noEmit`) passes clean.
...
- The real security boundary between roles is now in place (`docs/BACKLOG.md` section C is done); performance work for real data volume is not — see `docs/BACKLOG.md` section D. There's also no UI yet to set a shipment's planned depart date or mark it departed with a real date (Stream A section A) — a real, tracked gap, not a regression. Treat this as a hardening-in-progress platform, not a finished product.
