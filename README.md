...
This started as a **V1 skeleton** and has since gone through two hardening passes (Backlog Stream B: migration & cutover readiness; Backlog Stream A: wiring completion). The ledger-centric core, auth, and the Stock/Purchase Orders/Shipments/Money dashboards are built and tested, the schema/migration tooling is real (foreign keys, negative-stock validation, quarantine-not-abort migration handling), and every piece of backend logic V1's own review found "built, tested, but unreachable" now has a real router procedure and UI — but it has not yet run against real Control Tower data — see [Current Status](#current-status) below before assuming this is production-ready.
...
- 124 tests passing on the documented `pnpm test` command.
